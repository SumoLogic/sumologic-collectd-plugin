        ...
        for (ds_name, ds_type) in zip(self.ds_names, self.ds_types):
            if not self.meta:
                metrics.append('host=%s plugin=%s plugin_instance=%s type=%s type_instance=%s '
                               'ds_name=%s ds_type=%s  %f %d' %
                               (self.host, self.plugin, self.plugin_instance, self.type, self.type_instance,
                                ds_name, ds_type, self.values[i], self.time))
            else:
                metrics.append('host=%s plugin=%s plugin_instance=%s type=%s type_instance=%s '
                               'ds_name=%s ds_type=%s  test_meta_key=%s %f %d' %
                               (self.host, self.plugin, self.plugin_instance, self.type, self.type_instance,
                                ds_name, ds_type, self.meta['test_meta_key'], self.values[i], self.time))
            i += 1
        return metrics
